...
def pad_array(a, max_length, PAD=constants.constant['PAD']):
    """
    a (array[int32])
    """
...
def pad_arrays_keep_invp(src, label):
    ...
    print('src', src)
    print('idx', idx)
    # print('np.array(src)', np.array(src))
    ...
    # contiguous强制拷贝一份tensor，让它的布局和从头创建的一模一样，但是两个tensor完全没有联系。
...
class DataLoader():
    ...
    def load(self, max_num_line=0):
        ...
        for (s, t) in zip(srcstream, trgstream):
            ...
            # 同上，并且给轨迹加上起始标志
            t = [constants.constant['BOS']] + [int(x) for x in t.split()] + [constants.constant['EOS']]
            # s, t是一个轨迹的序列点数组
            # 返回1则插入成功，否则0
        ...
        if self.validate == True:
            ...
            for idx, (src, trg) in enumerate(zip(self.srcdata, self.trgdata)):
                ...
                self.srcdata[idx] = np.array(src)[shuffle]
                self.trgdata[idx] = np.array(trg)[shuffle]
    ...
    def getbatch_loader(self):
        if self.validate == True:
            ...
            # 取出指定索引的数据
            return pad_arrays_pair(src[idx],  # self.srcdata[bucket][idx]是 挑选的batch条轨迹数据
                                   trg[idx])
    ...
    def getbatch_keep_invp(self):
        ...
        ## select data from the bucket
        idx = np.random.choice(len(self.srcdata[bucket]), self.batch)  # 挑选的batch条轨迹数据的原始编号
        return pad_arrays_pair_keep_invp(self.srcdata[bucket][idx],  # self.srcdata[bucket][idx]是 挑选的batch条轨迹数据
                               self.trgdata[bucket][idx],
                               idx)


# class DataLoader_OnlyTrg():
#     """
#     只加载train.trg数据，不加载train.src数据-----暂时没用
#     trgfile: target file name
#     batch: batch size
#     validate: if validate = True return batch orderly otherwise return
#         batch randomly
#     """
#     def __init__(self, trgfile, batch, bucketsize, validate=False):
#         self.trgfile = trgfile # 训练集-原轨迹的
#         self.batch = batch
#         self.validate = validate
#         #self.bucketsize = [(30, 30), (30, 50), (50, 50), (50, 70), (70, 70)]
#         self.bucketsize = bucketsize
...
#     def load(self, max_num_line=0):
#         self.trgdata = [[] for _ in range(len(self.bucketsize))]
#         trgstream =  open(self.trgfile, 'r') # srcstream 训练集-带噪声的； trgstream 训练集-原轨迹的
#         num_line = 0
#         # for (s, t) in zip(trgstream, trgstream):
#         for t in trgstream:
#             t = [constants.BOS] + [int(x) for x in t.split()] + [constants.EOS]
#             num_line += self.insert(t)
#             if num_line >= max_num_line and max_num_line > 0: break
#             if num_line % 100000 == 0:
#                 print("Read line {}".format(num_line))
# 
...
#             self.trgdata = np.array(merge(*self.trgdata))
#             self.start = 0
#             self.size = len(self.trgdata)
#         else:
#             self.trgdata = list(map(np.array, self.trgdata))
#             self.allocation = list(map(len, self.trgdata))
#             self.p = np.array(self.allocation) / sum(self.allocation)
#         trgstream.close()
# 
#     def getbatch(self):
...
#             if self.start >= self.size:
#                 self.start = 0
#             return pad_arrays_pair(trg)
#         else:
#             ## select bucket
#             sample = np.random.multinomial(1, self.p) # 从8份中随机挑选一个，如第2个
#             bucket = np.nonzero(sample)[0][0] # 随机挑选的1份的序号，如3
#             ## select data from the bucket 从随机挑选的一份训练数据集中，随机挑选batch条轨迹数据
#             idx = np.random.choice(len(self.trgdata[bucket]), self.batch) # 挑选的batch条轨迹数据的编号
#             return pad_arrays_pair( # 返回挑选的batch条轨迹数据的srcdata和trgdata,每条轨迹的长度调整为一致
#                                    self.trgdata[bucket][idx])
...
class DataOrderScaner():
    ...
    def load(self, max_num_line=0):
        ...
        self.labels = np.array(self.labels)[self.shuffle]
        self.start = 0
...
class MyDataOrderScaner():
    def __init__(self, cell_trips, batch):
        self.cell_trips = cell_trips
        self.batch = batch
        self.srcdata = []
        # 指针
        self.start = 0
    ...
    def set_neighbor_data(self, cluster_data_neighbors):
        self.data_neighbors_before_shuffle = np.array(
            list(map(cluster_data_neighbors.get, range(len(cluster_data_neighbors)))))